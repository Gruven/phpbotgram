...
[`FlagGenerator`](https://api.phpbotgram.local/Gruven-PhpBotGram-Dispatcher-Flags-FlagGenerator.html) is the helper that converts a flag list into a normalised form for inspection. The dispatcher does not use it on the hot path — it's exposed for tooling and tests that want to assert "this handler has exactly these flags". Production middlewares should use the `Flags::getFlag` shortcut instead.
