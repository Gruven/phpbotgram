...
The variadic `$kwargs` parameter is deliberate: it signals to
`CallableObject::prepareKwargs` that the filter wants the *entire*
dispatcher kwargs bag (`bot`, `state`, `event_context`, …), not just
the keys it literally declares. Without the variadic the prepare step
intersects the bag against the parameter names and drops everything
else — which would silently break filters that need contextual data
the dispatcher injected.

The following filter accepts messages containing the word "hello" and
injects a `greeting` kwarg into the handler (adapted from
...
Pass filter instances to the `filters:` named argument of `register`.
...
The `$greeting` parameter arrives because `ContainsHelloFilter` returned
`['greeting' => 'Hello, friend!']`. The dispatcher merges every filter's
array return into the kwargs bag before calling the handler.

### Combining filters

Filters compose with three static helpers.

`Filter::all($f1, $f2)` builds an
[`AndFilter`](https://api.phpbotgram.local/Gruven-PhpBotGram-Filters-Logic-AndFilter.html)
...
that flips the accept/reject decision.
...
PHP cannot overload `&`/`|`/`~`, so we ship method forms — the result
composes identically to aiogram's `f1 & f2 | ~f3` syntax. The
static-helper form is more verbose than operator overloading but reads
identically once you're used to it.
...
command matching, callback prefix routing, etc. Global filters are appended via
[`$observer->filter(...)`](https://api.phpbotgram.local/Gruven-PhpBotGram-Dispatcher-Event-TelegramEventObserver.html#method_filter)
— e.g. `$dispatcher->message->filter(...)`.
...
filter, catch inside the filter's `__invoke` and return `false`:
...
The `Command` filter has one specific exception: failures inside `$bot->getMe()`
